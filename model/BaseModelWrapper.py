...
class BaseWrapper:
    ...
    def run(self, loader, train, interval=20):
        ...
        for i, inputs in enumerate(loader, 0):
            ...
            precision = precision_score(true_labels[1:], pred_labels[1:], pos_label=1)
            recall = recall_score(true_labels[1:], pred_labels[1:], pos_label=1)
            mcc = matthews_corrcoef(true_labels[1:], pred_labels[1:])
            ...
            if i % interval in [0] and i > 1:                                
                print('=> stage: {} | niter: {} | Loss: {:.4f} | Acc: {:.4f} | F1: {:.4f} | Pos: {:.4f} | Precision: {:.4f} | Recall: {:.4f} | MCC: {:.4f}'
                      .format(stage, i, np.nanmean(tot_loss), tot_acc, tot_f1, pos_count/tot_count,precision,recall,mcc))
                 
        return tot_acc, tot_f1, pos_count/tot_count, np.nanmean(tot_loss),precision,recall,mcc